...
import { FormRouterChannelApi, FormRouterEnvelopeApi } from "../api";
 ...
 */
export class FormRouterWebview {
  ...
  constructor(
    ...
    private readonly apiImpl: FormRouterChannelApi
  ) {}
  ...
  public open(pageId: string, opts: { onClose: () => void }) {
    ...
    webviewPanel.webview.html = `<!DOCTYPE html>
        ...
        <head>
          <style>
            html, body, div#form-envelope-app {
                margin: 0;
                border: 0;
                padding: 10%;
                height: 100%;
        ...
        <body>
        <div id="form-envelope-app" />
        <script src="${scriptSrc}"></script>
        </body>
        </html>`;

    const envelopeServer: EnvelopeServer<FormRouterChannelApi, FormRouterEnvelopeApi> = new EnvelopeServer(
      ...
      () =>
        envelopeServer.manager.clientApi.requests.form__init(
          { origin: envelopeServer.origin, envelopeServerId: envelopeServer.id },
          { name: this.envelopeLocator.name }
        )
    );
    ...
    this.context.subscriptions.push(
      webviewPanel.webview.onDidReceiveMessage(
        (msg) => {
          console.log(msg);
          return envelopeServer.receive(msg, this.apiImpl);
        },
